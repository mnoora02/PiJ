...
public class Supermarket {							
	...
	private PersonQueue personQueue; 				// its type will be PersonQueue and name personQueue - this will allow Supermarket to access it
	...
	public Supermarket() {							// now we have a reference to a PersonQueue obj (personQueue) - we can create a 
		this.personQueue = new PersonQueueImpl();	// constructor and use the obj. to fill out our two methods and we initialise our PersonQueueImpl
	}												// NB: constructors do no have a return type
													// on this occassion we do not need to pass anything in but we do want to create/initialise
													// our personQueue obj hence --> this.personQueueu = new PersonQueueImpl
	...
	public void addPerson(Person joePublic) {		// PARAM is of type Person - again you need a class Person
		this.personQueue.insert(joePublic); 		// the details for these methods must be created ie. in PersonQueue
	}												// NB: remember Supermaket is not an interface - hence we must provide actuals details for them
	...
	public Person servePerson() {					// return type is Person but could be void depending on what is required - here we will return
